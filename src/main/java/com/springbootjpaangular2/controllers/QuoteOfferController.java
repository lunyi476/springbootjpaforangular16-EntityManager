...
public class QuoteOfferController {
	private static final Logger logger = LoggerFactory.getLogger(QuoteOfferController.class);

	public static String DEFAULT_OWNER = "BAMBOOROSE";
	private QuoteOfferService quoteOfferService;

	@Autowired
	public void setQuoteOfferService(QuoteOfferService quoteOfferService) throws ParseException {
	...
	/**
	 * For dynamic Bean validation, Data Binding(user input to Bean) and event Bean
	 * field type conversion (besides annotation field type validation and
	 * conversion).
	 */
	@InitBinder
	private void initBinder(WebDataBinder binder) { // ServletRequestDataBinder
		 ...
		 * and @Pathvariable Or @RequestParam("request_no") in mapping method.
		 */
	...
	public ResponseEntity<String> login(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtils.isNotBlank(user) && StringUtils.isNotBlank(pass)) {
			...
			return new ResponseEntity<String>(resp.toString(), headers, HttpStatus.OK);
		} else {
	...
	/**
	 * @Pathvariable Or @RequestParam("request_no") Quotes reqNo, can trigger Custom
	 *               Editor which set request_no
	 */
	@ResponseBody
	@GetMapping(value = "/listquotes", produces = MediaType.APPLICATION_JSON_VALUE)
	public List<Quotes> listQuotes(HttpServletRequest request, HttpServletResponse response) {
	...
	public List<Quotes> listQuotesXml(HttpServletRequest request, HttpServletResponse response) {
		List<Quotes> all = quoteOfferService.listAllQuotes();
	...
	public ResponseEntity<String> listQuotesString(HttpServletRequest request, HttpServletResponse response)
			throws JsonProcessingException, IOException {
		List<Quotes> all = quoteOfferService.listAllQuotes();

		/**
		 * JSONObject outputJson = new JSONObject(); outputJson.put(CHNG_FIELDS_ARRAY,
		 * changeFields); outputJson.put(REPEAT_COUNTS, repeatingCount);
		 * outputJson.put(DOC_VIEW_ID, docViewId); outputJson.put(PAGE_NO, pageNo);
		 * return all;
		 **/
		...
		return new ResponseEntity<String>(xml, new HttpHeaders(), HttpStatus.OK);
	}

	@ResponseBody
	@GetMapping(value = "/listoffers", produces = MediaType.APPLICATION_JSON_VALUE)
	public List<Offers> listOffers(HttpServletRequest request, HttpServletResponse response) {
	...
	public Quotes getQuote(HttpServletRequest request, HttpServletResponse response)
			throws JsonMappingException, JsonParseException, IOException {
		String owner = request.getParameter("owner") == null ? DEFAULT_OWNER : request.getParameter("owner");
	...
	public ResponseEntity<String> delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		headers.add("Access-Control-Expose-Headers", "*");
		headers.add("message", messageResource.getMessage("quote.delete.respmessage", null, Locale.ENGLISH));
	...
	public static class XmlConverterHelper implements Serializable {
		...
		/**
		 * This method builds the message element with status, message_id, message_desc,
		 * and token as attributes
		 * 
		 * @param status
		 * @param id
		 * @param msg
		...
		public static String buildMessageTag(String status, String id, String msg, String token) {
			...
			outputXml.append("<message status=\"" + status + "\" message_id=\"" + id + "\" message_desc=\"" + msg + "\""
		...
		/**
		 * This method builds the message element with status, message_id and
		 * message_desc as attributes
		 * 
		 * @param status
		 * @param id
		 * @param msg
		...
		public static String getResponseXML(String strSuccessMsg, String strWarningMsg, String strErrorMsg) {
			StringBuilder outputXml = new StringBuilder(XmlConverterHelper.buildDocumentStartTag());

			if (strSuccessMsg != null) {
				outputXml.append(
						XmlConverterHelper.buildMessageTag(SUCCESS_STATUS, SUCCESS_HTTP_STATUS_CODE, strSuccessMsg));

			}
			if (strWarningMsg != null) {
				outputXml.append(
						XmlConverterHelper.buildMessageTag(WARNING_STATUS, WARNING_HTTP_STATUS_CODE, strWarningMsg));

			}
			if (strErrorMsg != null) {
				outputXml.append(
						XmlConverterHelper.buildMessageTag(FAILED_STATUS, FAILED_HTTP_STATUS_CODE, strErrorMsg));
